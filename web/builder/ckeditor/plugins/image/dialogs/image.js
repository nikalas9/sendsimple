	...
	var imageDialog = function( editor, dialogType ) {
			...
			var switchLockRatio = function( dialog, value ) {
					...
					if ( value == 'check' ) {
						if ( !dialog.userlockRatio && oImageOriginal.getCustomData( 'isReady' ) == 'true' ) {
							...
							dialog.lockRatio = true; // Default: unlock ratio
			...
			var resetSize = function( dialog ) {
					...
					if ( oImageOriginal.getCustomData( 'isReady' ) == 'true' ) {
                        ...
                        if(dialog.imageElement.$.className == 'image') {
                            var width = dialog.getValueOf( 'info', 'txtWidth' );
                            var heightPicture = Math.round( (width * oImageOriginal.$.height) / oImageOriginal.$.width );
                            widthField && widthField.setValue( width );
                            heightField && heightField.setValue( heightPicture );
                        }
                        else{
			...
			var numbering = function( id ) {
                ...
                imageEditor = numbering( 'previewImage' );
			...
			return {
				...
				onShow: function() {
                    ...
                    /*if(this.imageEditMode){
                        var textLinkId =  $('#'+this.getContentElement( 'info', 'txtUrl').domId).find('input').attr('id');
                        if(textLinkId){
                            $('#'+imageEditor).html( editor.lang.common.preview +
                                ' <a class="cke_dialog_ui_button" title="'+editor.lang.image.imageEditor+'" href="javascript:" onclick="return launchEditor(\''+textLinkId+'\',\''+previewImageId+'\');">'+
                                '<span class="cke_dialog_ui_button">'+
                                editor.lang.image.imageEditor+
                                '</span></a>');
                            $('#'+previewLinkId).attr('onclick',"return launchEditor('"+textLinkId+"','"+previewImageId+"');");
                        }
                    }*/
				},
				onOk: function() {
					...
					elements: [
								...
								onChange: function() {
									...
									if ( newUrl.length > 0 ) //Prevent from load before onShow
									{
										...
										if ( dialog.preview ) {
                                            ...
                                            /*var textLinkId = this._.inputId; 17.05.2016
                                            if(textLinkId){
                                                $('#'+imageEditor).html( editor.lang.common.preview +
                                                ' <a class="cke_dialog_ui_button" title="'+editor.lang.image.imageEditor+'" href="javascript:" onclick="return launchEditor(\''+textLinkId+'\',\''+previewImageId+'\');">'+
                                                '<span class="cke_dialog_ui_button">'+
						...
						children: [
							...
							children: [
								...
								style: 'width:95%;',
								html: '<div>' +
                                    '<p style="margin-bottom: 4px;" id="'+imageEditor+'">' + editor.lang.common.preview + '</p>'+
