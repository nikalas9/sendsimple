...
default_theme={boxShadow:["",0,1,0,0,"ffe0b5"],
...
_parent.doksoft_parse=function(a){
	...
	var g=function(j){
		...
		var p=/(.*?)rgb\((\d+),\s*(\d+),\s*(\d+)\)/i.exec(j),o=parseInt(p[2],10).toString(16),n=parseInt(p[3],10).toString(16),i=parseInt(p[4],10).toString(16);
		return((o.length==1?"0"+o:o)+(n.length==1?"0"+n:n)+(i.length==1?"0"+i:i));
	};
	var d=[/box-shadow:[\s]?(inset)?([\s\-0-9]+)px ([\s\-0-9]+)px ([\s\-0-9]+)px ([\s\-0-9]+)px (#[0-9abcdef]+|rgb\([\s0-9]+,[\s0-9]+,[\s0-9]+\));/i,/background-color:[\s]?(#[0-9abcdef]+|rgb\([\s0-9]+,[\s0-9]+,[\s0-9]+\));/i,/background:[\s]?linear-gradient\(to bottom, (#[0-9abcdef]+|rgb\([\s0-9]+,[\s0-9]+,[\s0-9]+\)) 5%, (#[0-9abcdef]+|rgb\([\s0-9]+,[\s0-9]+,[\s0-9]+\)) 100%\);/i,/background:[\s]?linear-gradient\([\s]?(rgb\([\s0-9]+,[\s0-9]+,[\s0-9]+\))[\s]?[0-9]+\%,[\s]?(rgb\([\s0-9]+,[\s0-9]+,[\s0-9]+\))[\s]?[0-9]+%/i,/border-radius:([\s\-0-9]+)px;/i,/border:([\s\-0-9]+)px solid (#[0-9abcdef]+|rgb\([\s0-9]+,[\s0-9]+,[\s0-9]+\));/i,/;[\s]?color:[\s]?(#[0-9abcdef]+|rgb\([\s0-9]+,[\s0-9]+,[\s0-9]+\));/i,/font-family:([^;]+);/i,/font-size:([\s\-0-9]+)px;/i,/font-weight:[\s]?(bold|normal);/i,/font-style:[\s]?(italic|normal);/i,/padding:([\s\-0-9]+)px ([\s\-0-9]+)px;/i,/text-shadow:([\s\-0-9]+)px ([\s\-0-9]+)px ([\s\-0-9]+)px (#[0-9abcdef]+|rgb\([\s0-9]+,[\s0-9]+,[\s0-9]+\));/i,],l=["boxShadow","background","gradient","gradient","borderRadius","border","color","fontFamily","fontSize","fontWeight","fontStyle","padding","textShadow"],f={},h=false,b="";
...
_parent.getButtonLink=function(){
	return _("button-href").value;
};
_parent.getButtonText=function(){
	return _("button-text").value;
};

_parent.getResultButton=function(){
	return'<a style="'+_parent.doksoft_remix(_parent._theme)+'" href="'+_("button-href").value+'">'+_("button-text").value+"</a>";
};
...
(function(){
	...
	_("setting-fontFamily-1").onchange = function(){
		this.value=_parent._theme['fontFamily'] = [$(this).val()];
		_parent.doksoft_recalc(_parent._theme);
	};
