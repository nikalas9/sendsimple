 ...
 */
namespace app\components\MailBounceHandler;
...
class Handler
{
    ...
    private function processMailParsing($token, $content)
    {
        ...
        if($cwsMbhMail->isProcessed()){
            $messageId = $this->parseMessageId($bodySections['returned']);
            $cwsMbhMail->setMessageId($messageId);
        }
     ...
     * @return array
     */
    private static function parseMessageId($content)
    {
        $rt = self::parseLines($content);
        if(isset($rt['Message-id']) and $rt['Message-id']){
            $rt['Message-id'] = str_replace('<','',$rt['Message-id']);
            $rt['Message-id'] = str_replace('>','',$rt['Message-id']);
            return trim($rt['Message-id']);
        }
        return false;
    }
