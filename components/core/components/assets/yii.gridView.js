    ...
    var methods = {
                ...
                $(document).off('change.yiiGridView keydown.yiiGridView', settings.filterSelector)
                    .on('change.yiiGridView keydown.yiiGridView', settings.filterSelector, function (event) {
                        ...
                        if( event.target.hasAttribute('autocomplete') ) {
                            if (event.type === 'change' && event.isTrigger == undefined) {
                                return;
                            }
                        }
        ...
        applyFilter: function () {
            ...
            $.each(yii.getQueryParams(settings.filterUrl), function (name, value) {
                ...
                if( $(settings.filterSelector).filter('[name="'+name+'"]').filter('[type=checkbox]').length ){
                    if( $(settings.filterSelector).filter('[name="'+name+'"]').filter('[type=checkbox]').is(':checked') == false ){
                        delete data[name];
                    }
                }
            });
