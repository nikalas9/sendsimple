<?php
namespace core\components\gridBulkActions;
...
class GridBulkActions extends Widget
{
	...
	protected function setDefaultOptions()
	{
		if ( ! $this->actions )
		{
            ...
            if ( User::canRoute( Url::toRoute('bulk-activate') ) )
            {
                $this->actions[ Url::to(['bulk-activate']) ] = GridBulkActions::t('app', 'Activate');
            }
            if ( User::canRoute( Url::toRoute('bulk-deactivate') ) )
            {
                $this->actions[ Url::to(['bulk-deactivate']) ] = GridBulkActions::t('app', 'Deactivate');
            }

            if ( User::canRoute( Url::toRoute('bulk-delete') ) )
            {
                $this->actions[ '----' ] = [
                    Url::to(['bulk-delete'])=>GridBulkActions::t('app', 'Delete'),
                ];
		...
		$(document).off('click', '.grid-bulk-ok-button').on('click', '.grid-bulk-ok-button', function () {
			...
			$.post(list.val(), $(_t.data('grid') + ' [name="selection[]"]').serialize() )
				.done(function(){
					...
					$.pjax.reload({container: _t.data('pjax'), timeout: 3000});
				});
		});
JS;
